...
// Shoulder Abd/Add — handle L normally; R corrected
if (/^sh_aa_/.test(key)) {
  const isLeft = key.endsWith('_l');
  if (isLeft) {
    // Left: + = Abduction, − = Adduction (normal)
    return (sign > 0) ? COLLIM.shAbdLMax : COLLIM.shAddLMax;
  } else {
    // ✅ Right side corrected: + = Abduction, − = Adduction
...
if (cfg.key === 'hip_aa_r'){
  ...
  return out;
    ...
    // --- Left Hip fix: rename only ---
if (cfg.key === 'hip_irer_l') {
  push(cfg.pair.replace(/—.*$/, '— External Rotation'), +1, maxPos);
  push(cfg.pair.replace(/—.*$/, '— Internal Rotation'), -1, absMin);
  return out;
}
...
/* ============================================================
   AUTO-ACTION EXPORTS — allow autoActions.js to import your rig
   ============================================================ */
window.ROMETRICS_LAB = {
  UPPER,
  LOWER,
  COLLIM,
  cfgFromKey,
